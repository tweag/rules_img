...
import (
	...
	build_event_stream_proto "github.com/tweag/rules_img/pkg/proto/bazel/src/main/java/com/google/devtools/build/lib/buildeventstream"
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				...
				log.Println("Build event stream closed by client")
				return nil
			}
		...
		if err := proto.Unmarshal(bazelEvent.Value, &buildEvent); err != nil {
			log.Printf("Error unmarshaling build event: %v", err)
		} else {
...
func (b *BES) processBuildEvent(event *build_event_stream_proto.BuildEvent, tracker tracker, comittErrGroup *errgroup.Group, commitCtx context.Context) error {
	// Check the event ID to determine the event type
	if event.Id == nil {
	...
	case *build_event_stream_proto.BuildEventId_TargetConfigured:
		...
		log.Printf("Target configured: %s\n", configured.String())
		...
		for _, childId := range event.GetChildren() {
			...
			log.Printf("Waiting for TargetCompleted: %s\n", childId.String())
		}
	...
	case *build_event_stream_proto.BuildEventId_TargetCompleted:
		...
		log.Printf("Processing TargetCompleted event for ID: %s\n", idHash)
		if !completed.Success {
			log.Printf("Target %s failed to complete successfully\n", idHash)
			return nil // we only care about successful completions
		}
		targetCompletedId := event.Id.GetTargetCompleted()
		log.Printf("Target completed: %s (%s)\n", targetCompletedId.Label, targetCompletedId.Configuration.Id)
		...
		for _, group := range completed.OutputGroup {
			...
			log.Printf("Found push JSON descriptor for target %s: %s\n", idHash, pushJSONDescriptor.String())
			...
			comittErrGroup.Go(func() error {
				log.Printf("Starting background commit for target %s (digest: %s)", idHash, digest)
				if err := b.syncer.Commit(commitCtx, digest, length); err != nil {
					log.Printf("Background commit failed for target %s: %v", idHash, err)
					return fmt.Errorf("failed to commit image for target %s: %w", idHash, err)
				}
				log.Printf("Background commit completed for target %s", idHash)
				return nil
			})
		}
	}
